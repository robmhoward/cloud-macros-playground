...
app.get('/api/me/scripts', function(request, response) {
    response.send(getServiceData.getMyScripts());
    response.end();
});

app.get('/api/org/scripts', function(request, response) {
    response.send(getServiceData.getOrganizationScripts());
    response.end();
});

app.get('/api/me/favorites', function(request, response) {
    response.send(getServiceData.getMyFavoriteScripts());
    response.end();
});

app.get('/api/scripts', function(request, response) {
    response.send(getServiceData.getGlobalScripts());
    response.end();
});

app.get('/api/me', function(request, response) {
    response.send(getServiceData.getMyProfile());
    response.end();
});

function catchCode(request, response, authConfig, scopes) {
...
app.get('/catchCode', function(request, response) {
	catchCode(request, response, "AAD", null, "https://graph.microsoft.com");
});
